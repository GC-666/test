<template>
	<tm-app>
		...
		<tm-sheet :round="5" :shadow="0" :margin="[20,0]" :padding="[20,10]">
			<tm-divider :color="bgColor" :font-color="bgColor" :border="10" :font-size="32"
				:label="`${composeFind.name}合成通道`"></tm-divider>
			...
			<view class="flex pt-20 pb-20">
				...
				<view class="" style="width: 375rpx;">
					<tm-text :font-size="22" _class="text-weight-b"
						:label="composeFind.limitNumber-composeFind.useFalseNumber"></tm-text>
				</view>
			</view>
			<tm-divider color="grey" :margin="[1,1]"></tm-divider>
			<view class="flex pt-20 pb-20">
			...
			<view class="flex pt-20 pb-20">
				...
				<view class="" style="width: 375rpx;">
					<tm-text :font-size="22" _class="text-weight-b"
						:label="DateUtils.formatDateTime(composeFind.startTime)"></tm-text>
				</view>
			</view>
		</tm-sheet>
		<tm-sheet :round="5" :shadow="0" :margin="[20,20]" :padding="[20,10]">
			<view class="flex">
				<view class="flex flex-center pt-20 pb-20">
					<tm-text color="#808080" :font-size="30" _class="text-weight-b" label="作品描述 :"></tm-text>
					<tm-text class="ml-10" :font-size="22" label="你可能合成到以下作品中的一个"></tm-text>
				</view>
			</view>
			<view class="flex flex-wrap">
				<view class="flex flex-center" style="width:25%;height:146rpx;overflow: hidden;margin-top: 10rpx;"
					v-for="(data,index) in composeFind.collectionsList">
		...
		<tm-sheet :round="5" :shadow="0" :margin="[20,0]" :padding="[20,10]">
			...
			<view class="flex flex-wrap">
				<view class="flex flex-center" style="width:50%;" v-for="(data,index) in composeFind.materialList">
					<tm-sheet :shadow="10">
						<view class="" :style="`background-image: url(${data.collImg});background-size: 100% 100%;
										width:300rpx;height:300rpx;border-radius: 20rpx;overflow: hidden;`" @click="selectMaterial(index)">
		...
		<tm-sheet :round="5" :shadow="0" :margin="[20,10]" :padding="[20,10]">
			<view class="pt-20 pb-20">
				<tm-text :font-size="30" _class="text-weight-b" label="合成须知"></tm-text>
				<tm-text class="mt-10" style="text-indent: 1.8em;letter-spacing:2rpx;letter-spacing:4rpx;"
					:font-size="22"
					label="数字藏品为虚拟数字商品，而非实物，仅限实名认证为年满14周岁的中国大陆用户购买。数字藏品的版权由发行方或原创者拥有，除另行取得版权拥有者书面同意外，不得将数字藏品用于任何商业用途，不支持退换。本商品源文件不支持本地下载。请勿对数字藏品进行炒作、场外交易、欺诈，或以任何其他非法方式进行使用">
				</tm-text>
			</view>
		</tm-sheet>
		<tm-button @click="composeCompose" :margin="[40,40]" form-type="submit" :fontSize="38" linearDeep="accent" block
			label="合成"></tm-button>
				...
				<tm-cell :margin="[0, 0]" :titleFontSize="30">
					...
					<template v-slot:right>
						<tm-checkbox-group @change="setSyntheticMaterialList(i)">
							<tm-checkbox :defaultChecked="data.isSelect" :size="30" :round="10" :value="i">
							</tm-checkbox>
						</tm-checkbox-group>
					</template>
				</tm-cell>
...
<script setup>
	...
	const composeCompose = (e) => {
		...
		Activity.composeCompose({
			...
			uni.navigateTo({
				url: '/pages/activity/syn/synDetailsSuccess?config=' + encodeURIComponent(JSON
					.stringify(res)),
				success: function() {}
			})
		})
