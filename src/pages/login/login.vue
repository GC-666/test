<template>
	<tm-app ref="app" color="grey-5">
		...
		<tm-form @submit="confirm" v-model="show" transprent>
			...
			<tm-form-item required field="userAccount" :rules="[{required:true,message:'请输入手机号'}]">
				<tm-input placeholder="请输入手机号" :inputPadding="[0,0]" v-model="show.userAccount" :transprent="true"
			...
			<tm-form-item required field="userPassword" :rules="[{required:true,message:'请输入手机号'}]">
				<tm-input placeholder="请输入密码" password :inputPadding="[0,0]" v-model="show.userPassword" :transprent="true"
					:showBottomBotder="false">
				</tm-input>
			</tm-form-item>
